...
# ========================
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    orders = Order.joins(:order_items).where('order_items.item_id': @item_4.id, user: @user_2)
    # -----------------------------------------------------------
...
# ========================
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    # ------------------ ActiveRecord S olution ----------------------
     ordered_items_names = Item.joins(:order_items).group(:id).pluck(:name)
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    data = User.joins(orders: :order_items).select("users.name as user_name, orders.id as order_id, orders.amount / count(order_items.id) as avg_item_cost").group("orders.id").order("name desc, avg_item_cost")
    # ---------------------------------------------------------------
